...
class ConvolutionLayer(_Layer):
    ...
    def forward(self, input):
        ...
        output = np.zeros((self.out_channels, out_h, out_w, batch_size))
            ...
            for i in range(self.out_channels):
                for h in range(out_h):
                    for w in range(out_w):
                        ...
                        # Correct broadcasting: (in_channels, kernel_size, kernel_size) * (in_channels, kernel_size, kernel_size)
                        output[i, h, w, b] = np.sum(input_slice * self.W[i]) + self.b[i]

        return output

    def backward(self, output_grad):
        # output_grad shape: (out_channels, out_h, out_w, batch_size)
